...
public class ExecutionManagerImpl implements ExecutionManager {
    ...
    public List<TaskSubmitResult> submitApplication(List<Rule> rules, String nodeName, String createTime, String user, String database
        ...
        for (String clusterName : clusterNameMap.keySet()) {
            ...
            // Divide rule into tasks（将应用切分成多个任务）
            ...
            for (DataQualityTask task : tasks) {
                DataQualityJob job = templateConverterFactory.getConverter(task)//对任务进行检查，是sql还是spark等任务
                        .convert(task, date, setFlag, execParams, runDate.toString(), clusterInfo.getClusterType(), dataSourceMysqlConnect);//SqlTemplateConverter
            ...
            for (DataQualityJob job : jobList) {
                ...
                //分别往不同版本Linkis提交作业
                if (clusterInfo.getClusterType().endsWith(LINKIS_ONE_VERSION)) {//1.01新版本
